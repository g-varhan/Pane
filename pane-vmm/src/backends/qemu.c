...
static void qemu_args_add_machine(qemu_argv_t *qa,
                                  const pane_vmm_config_t *cfg) {
  (void)cfg;
  qemu_argv_push(qa, "qemu-system-x86_64");
  qemu_argv_push(qa, "-enable-kvm");
}

static void qemu_args_add_cpu_mem(qemu_argv_t *qa,
                                  const pane_vmm_config_t *cfg) {
  char mem_str[64];
...
static void qemu_args_add_disk(qemu_argv_t *qa, const pane_vmm_config_t *cfg) {
  if (cfg->disk_path && strlen(cfg->disk_path) > 0) {
    ...
    char drive_arg[1024];
    snprintf(drive_arg, sizeof(drive_arg), "file=%s,format=%s,if=virtio",
             cfg->disk_path, fmt);
...
static void qemu_args_add_net(qemu_argv_t *qa, const pane_vmm_config_t *cfg) {
  if (cfg->virtio_net) {
    ...
    if (cfg->net_bridge && strlen(cfg->net_bridge) > 0) {
      snprintf(netdev_arg, sizeof(netdev_arg), "bridge,id=net0,br=%s",
               cfg->net_bridge);
    } else {
      snprintf(netdev_arg, sizeof(netdev_arg),
               "tap,id=net0,ifname=pane-%s-tap0,script=no,downscript=no",
               vm_id);
    }
...
static void qemu_args_add_kernel(qemu_argv_t *qa,
                                 const pane_vmm_config_t *cfg) {
...
static int send_qmp_cmd(int fd, const char *cmd, char *resp_out,
                        size_t max_len) {
...
int pane_vm_setup_qemu_mode(pane_vm_t *vm, const pane_vmm_config_t *config,
                            const char *qmp_socket_path) {
  if (!vm || !config || !qmp_socket_path)
    return -EINVAL;
  ...
  if (last_slash) {
    ...
    snprintf(log_path, sizeof(log_path), "%.*s/qemu-%s.log", dir_len,
             qmp_socket_path, config->vm_id);
  } else {
  ...
  // Write pid to a file so that assume_running/assume_frozen can find it
  char pid_path[1024];
  if (access("/run/pane", W_OK) == 0) {
    snprintf(pid_path, sizeof(pid_path), "/run/pane/qemu-%s.pid",
             config->vm_id);
  } else {
    snprintf(pid_path, sizeof(pid_path), "/tmp/pane/qemu-%s.pid",
             config->vm_id);
  }
...
int pane_vm_qemu_suspend(pane_vm_t *vm) {
  if (!vm || vm->backend != PANE_BACKEND_QEMU || vm->qmp_fd < 0)
    return -EINVAL;

  char buf[1024];
  int ret =
      send_qmp_cmd(vm->qmp_fd, "{\"execute\":\"stop\"}\n", buf, sizeof(buf));
  if (ret < 0)
    return ret;
...
int pane_vm_qemu_resume(pane_vm_t *vm) {
  ...
  char buf[1024];
  int ret =
      send_qmp_cmd(vm->qmp_fd, "{\"execute\":\"cont\"}\n", buf, sizeof(buf));
  if (ret < 0)
    return ret;
...
int pane_vm_qemu_query_status(pane_vm_t *vm, char *status_out, size_t max_len) {
  if (!vm || vm->backend != PANE_BACKEND_QEMU || vm->qmp_fd < 0 || !status_out)
    return -EINVAL;
...
int pane_vm_reconstruct_qemu(pane_vm_t **vm_out, const char *vm_id,
                             int qemu_pid, const char *qmp_socket_path) {
  if (!vm_out || !vm_id || !qmp_socket_path)
    return -EINVAL;
  ...
  vm->backend = PANE_BACKEND_QEMU;
  vm->qemu_pid = qemu_pid;
  vm->qmp_path = strdup(qmp_socket_path);
  vm->kvm_fd = -1;
  vm->vm_fd = -1;
  vm->qmp_fd = -1;

  // Connect to QMP Unix socket to populate qmp_fd
  int client_fd = -1;
  struct sockaddr_un addr;
  memset(&addr, 0, sizeof(addr));
  addr.sun_family = AF_UNIX;
  ...
  if (client_fd >= 0) {
    vm->qmp_fd = client_fd;
    // Do QMP capabilities handshake
    char buf[1024];
    if (read_qmp_greeting(client_fd, buf, sizeof(buf)) == 0) {
      send_qmp_cmd(client_fd, "{\"execute\":\"qmp_capabilities\"}\n", buf,
                   sizeof(buf));
    }
  }
