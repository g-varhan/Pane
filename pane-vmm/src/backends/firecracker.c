...
int pane_vm_setup_firecracker_mode(pane_vm_t *vm, uint32_t vcpu_id, uint64_t entry_point) {
    ...
    gdt[1] = 0x00af9b000000ffffULL;        // CS: 64-bit, present, code exec/read, ring 0, L=1
    gdt[2] = 0x00cf93000000ffffULL;        // DS/SS/ES: 64-bit, present, data read/write, ring 0
    ...
    for (int i = 0; i < 512; i++) {
        pd[i] = (i * 2 * 1024 * 1024ULL) | 0x83; // Present + Writable + PageSize (2MB)
    }
