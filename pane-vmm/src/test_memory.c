...
int main(void) {
  ...
  // We'll use slot 0, guest phys addr 0x10000000, size 64MB, userspace addr
  // 0x20000000 Note: We are not actually backing this memory, so the ioctl
  // might fail if the address is not accessible. But we are only testing the
  // function call and error handling. We'll use MAP_ANONYMOUS | MAP_PRIVATE to
  // allocate some userspace memory for the test.
  void *userspace = mmap(NULL, 64 * 1024 * 1024, PROT_READ | PROT_WRITE,
  ...
  ret = pane_vm_set_user_memory_region(vm, 0, 0x10000000, 64 * 1024 * 1024,
  ...
  ret =
      pane_vm_set_user_memory_region(vm, 1, 0x20000000, 32 * 1024 * 1024,
                                     (uint64_t)userspace + 64 * 1024 * 1024, 0);
  if (ret != 0) {
