...
int pane_uring_init(pane_vm_t *vm, uint32_t queue_depth) {
    if (!vm) return -EINVAL;
    if (vm->ring) return 0; // Already initialized
...
int pane_uring_submit_read(pane_vm_t *vm, int fd, void *buf, uint32_t len, uint64_t offset, void *user_data) {
    if (!vm || !vm->ring) return -EINVAL;
...
int pane_uring_submit_write(pane_vm_t *vm, int fd, const void *buf, uint32_t len, uint64_t offset, void *user_data) {
    if (!vm || !vm->ring) return -EINVAL;
...
int pane_uring_poll_completions(pane_vm_t *vm, void **user_data_out, int32_t *res_out) {
    if (!vm || !vm->ring) return -EINVAL;
