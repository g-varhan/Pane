...
static int pane_console_handle_mmio(struct virtio_mmio_dev *dev_base, uint64_t phys_addr, uint8_t *data, uint32_t len, uint8_t is_write);
...
int pane_handle_mmio(pane_vm_t *vm, uint64_t phys_addr, uint8_t *data, uint32_t len, uint8_t is_write) {
    ...
    for (int i = 0; i < vm->virtio_dev_count; i++) {
        ...
        if (dev && phys_addr >= dev->base_addr && phys_addr < dev->base_addr + dev->size) {
...
static void virtio_blk_process_queue(struct virtio_mmio_blk *dev) {
    ...
    while (q->last_avail_idx != avail_ring->idx) {
        ...
        struct virtio_blk_req_hdr *hdr = pane_vm_gpa_to_hva(dev->vm, desc_table[curr].addr);
        ...
        uint8_t *status_buf = pane_vm_gpa_to_hva(dev->vm, desc_table[status_desc_idx].addr);
        ...
        if (hdr->type == VIRTIO_BLK_T_IN) {
            extern int pane_uring_submit_read(pane_vm_t *vm, int fd, void *buf, uint32_t len, uint64_t offset, void *user_data);
            io_ret = pane_uring_submit_read(dev->vm, dev->disk_fd, data_buf, data_len, file_offset, dev);
        } else if (hdr->type == VIRTIO_BLK_T_OUT) {
            extern int pane_uring_submit_write(pane_vm_t *vm, int fd, const void *buf, uint32_t len, uint64_t offset, void *user_data);
            io_ret = pane_uring_submit_write(dev->vm, dev->disk_fd, data_buf, data_len, file_offset, dev);
        } else {
        ...
        if (io_ret == 0) {
            ...
            extern int pane_uring_poll_completions(pane_vm_t *vm, void **user_data_out, int32_t *res_out);
            
            while (1) {
                int poll_ret = pane_uring_poll_completions(dev->vm, &completed_dev, &bytes_transferred);
...
static int pane_blk_handle_mmio(struct virtio_mmio_dev *dev_base, uint64_t phys_addr, uint8_t *data, uint32_t len, uint8_t is_write) {
    ...
    if (is_write) {
        ...
        switch (offset) {
            ...
            case VIRTIO_MMIO_QUEUE_DESC_LOW: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].desc_gpa = (dev->queues[0].desc_gpa & 0xffffffff00000000ULL) | val;
                }
                break;
            }
            case VIRTIO_MMIO_QUEUE_DESC_HIGH: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].desc_gpa = (dev->queues[0].desc_gpa & 0x00000000ffffffffULL) | ((uint64_t)val << 32);
                }
                break;
            }
            case VIRTIO_MMIO_QUEUE_AVAIL_LOW: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].avail_gpa = (dev->queues[0].avail_gpa & 0xffffffff00000000ULL) | val;
                }
                break;
            }
            case VIRTIO_MMIO_QUEUE_AVAIL_HIGH: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].avail_gpa = (dev->queues[0].avail_gpa & 0x00000000ffffffffULL) | ((uint64_t)val << 32);
                }
                break;
            }
            case VIRTIO_MMIO_QUEUE_USED_LOW: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].used_gpa = (dev->queues[0].used_gpa & 0xffffffff00000000ULL) | val;
                }
                break;
            }
            case VIRTIO_MMIO_QUEUE_USED_HIGH: {
                if (dev->queue_sel == 0) {
                    dev->queues[0].used_gpa = (dev->queues[0].used_gpa & 0x00000000ffffffffULL) | ((uint64_t)val << 32);
                }
                break;
            }
            default:
