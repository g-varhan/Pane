...
int main(void) {
  ...
  // Check for fd leak: the number of FDs should be the same as initial (within
  // reason) Note: there might be some internal fds opened by the kernel, but we
  // expect no leak from our code. We'll just check that we didn't leak a large
  // number.
  if (after_destroy_fds >
      initial_fds + 10) { // allow a small slack for things like stderr, etc.
