...
int main(int argc, char **argv) {
  ...
  if (!use_payload) {
    ...
    printf("No kernel image specified. Running embedded bare-metal test "
           "payload in Real Mode...\n");
  }
  ...
  if (!use_payload) {
    ...
    const char *cmdline =
        "console=none earlyprintk=serial virtio_mmio.device=512@0x10000000:5 "
        "root=/dev/ram0 rw";
