...
// VM Backend Types
typedef enum { PANE_BACKEND_NATIVE = 0, PANE_BACKEND_QEMU } pane_backend_t;
...
// Configure the VM for direct 64-bit boot (Firecracker Mode), mapping GDT and
// 4-level identity page tables. Returns 0 on success, negative errno on
// failure.
int pane_vm_setup_firecracker_mode(pane_vm_t *vm, uint32_t vcpu_id,
                                   uint64_t entry_point);

// Set up a virtio-mmio console device at the specified physical address range
// and IRQ. Returns 0 on success, negative errno on failure.
int pane_vm_setup_virtio_mmio(pane_vm_t *vm, uint64_t base_addr, uint64_t size,
                              int irq);

// Set up a virtio-mmio block device at the specified physical address range,
// IRQ, and backing file path. Returns 0 on success, negative errno on failure.
...
// Configure the VM for QEMU mode, spawning QEMU with KVM acceleration and QMP
// socket. Returns 0 on success, negative errno on failure.
int pane_vm_setup_qemu_mode(pane_vm_t *vm, const pane_vmm_config_t *config,
                            const char *qmp_socket_path);

// Reconstruct a QEMU VM structure for an already running process.
