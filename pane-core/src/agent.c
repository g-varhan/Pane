...
static int write_frame(int sock, uint8_t type, const void *buf, uint32_t len) {
  ...
  if (write_all(sock, header, 5) != 5)
    return -1;
  if (len > 0 && write_all(sock, buf, len) != (ssize_t)len)
    return -1;
  return 0;
}
...
static int parse_request(const char *json, char *command, size_t max_cmd_len,
    ...
    if (args_colon) {
      ...
      if (start_bracket && end_bracket && start_bracket < end_bracket) {
        ...
        while (p < end_bracket && *arg_count < MAX_ARGS - 1) {
          ...
          if (!q1 || q1 >= end_bracket)
            break;
          q1++;
          const char *q2 = strchr(q1, '"');
          if (!q2 || q2 > end_bracket)
            break;
...
static void handle_client(int client_fd) {
  ...
  while (total_read < MAX_JSON_SIZE - 1) {
    ssize_t r =
        read(client_fd, json_buf + total_read, MAX_JSON_SIZE - 1 - total_read);
    if (r < 0) {
      if (errno == EINTR)
        continue;
