use pane_core::vm::{Vm, Spawning, Running, Frozen, Dead};
...
async fn test_native_full_lifecycle() {
    match pane_core::SafeVm::create() {
        Ok(safe_vm) => {
            ...
            let running_vm: Vm<Running> = vm.start().await.expect("Failed to transition to Running");

            // 3. Transition: Running -> Frozen
            let frozen_vm: Vm<Frozen> = running_vm.freeze().await.expect("Failed to transition to Frozen");

            // 4. Transition: Frozen -> Running
            let running_vm2: Vm<Running> = frozen_vm.resume().await.expect("Failed to transition back to Running");

            // 5. Transition: Running -> Dead
            let dead_vm: Vm<Dead> = running_vm2.destroy().await.expect("Failed to transition to Dead");
            assert_eq!(dead_vm.id(), "native-state-test");
        }
        Err(e) => {
            println!("Skipping native state machine lifecycle test. KVM not accessible: {:?}", e);
        }
    }
}
